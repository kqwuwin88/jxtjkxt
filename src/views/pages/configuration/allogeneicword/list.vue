    ...
    <div class="init-content-lay" flex="dir:top">
        <div class="toolbar-lay" flex-box="0">
            <el-form inline :model="formInline">
                <el-form-item label="">
                    <el-input placeholder="异体字" v-model.trim="formInline.srcWord" />
                </el-form-item>
                <el-form-item label="">
        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            <div class="table-wrap" flex-box="1">
                <el-table :data="tableData" class="table-border" ref="multipleTable" height="100%">
                    <el-table-column prop="srcWord" label="源字符" width=""></el-table-column>
                    <el-table-column prop="destWord" label="目标字符" width="120"></el-table-column>
        ...
        <el-dialog :visible.sync="dialogEditVisible" v-if="dialogEditVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon :name="this.dialogEditForm.id ? 'edit' : 'plus'" class="vsd-mr-5 m-color"/>{{this.dialogEditForm.id ? '修改异体字' : '添加异体字'}} </div>
            <el-form :model="dialogEditForm" label-width="80px" label-position="left" ref="dialogEditForm">
                <el-form-item label="源字符" prop="srcWord" :rules="{ required: true, message: '请输入源字符', trigger: 'blur' }">
                    <el-input v-model.trim="dialogEditForm.srcWord" placeholder="请输入源字符" show-word-limit></el-input>
                </el-form-item>
                <el-form-item label="目标字符" prop="destWord" :rules="{ required: true, message: '请输入目标字符', trigger: 'blur' }">
                    <el-input v-model.trim="dialogEditForm.destWord" placeholder="请输入目标字符" show-word-limit></el-input>
                </el-form-item>
        ...
        <el-dialog :visible.sync="dialogImportVisible" v-if="dialogImportVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon name="download" class="vsd-mr-5 m-color"/>导入异体字</div>
...
export default {
	...
	methods:{
        ...
        dnTemplate(){
            window.location.href=process.env.VUE_APP_APR+'/downloadTemplate?templateType=6';
        },
        importHandle(){
            this.fileName = ''
        ...
        submitImportDialog(){
            ...
            this.$refs.importForm.validate((valid) => {
                if (valid) {
                    this.$api.importSpecialword(this.dialogFormData).then( res => {
        ...
        exoprtList(){
            ...
            window.location.href=process.env.VUE_APP_APR + 'exportSpecialword';
        },
        addRow(row){
        ...
        submitEditDialog(){
            this.$refs.dialogEditForm.validate((valid) => {
                if (valid) {
                    this.$api.addSpecialword(this.dialogEditForm).then( res => {
        ...
        delRow(row){
      		this.$confirm('是否确认删除吗？', '提示', {
				...
				this.$api.deleteSpecialword({id:row.id}).then( res => { 
		...
		queryList(){
            this.tableData = []
			this.$api.specialList({...this.formInline,page:this.page}).then( res => { 
