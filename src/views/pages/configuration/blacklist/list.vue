<template>
    <div class="init-content-lay" flex="dir:top">
        ...
        <el-dialog :visible.sync="dialogEditVisible" v-if="dialogEditVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon :name="this.dialogEditForm.id ? 'edit' : 'plus'" class="vsd-mr-5 m-color"/>{{this.dialogEditForm.id ? '修改黑名单' : '添加黑名单'}} </div>
            <el-form :model="dialogEditForm" label-width="80px" label-position="right" ref="dialogEditForm">
                ...
                <el-form-item label="用户类别" prop="userType" :rules="{ required: true, message: '请选择用户类别', trigger: 'change' }">
                    <el-radio-group v-model="dialogEditForm.userType">
                        <el-radio :label="0">系统黑名单</el-radio>
                        <el-radio :label="1">端口黑名单</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="端口" prop="srcAddr" v-if="dialogEditForm.userType === 1" :rules="{ required: true, message: '请输入端口', trigger: 'change' }">
        ...
        <el-dialog :visible.sync="dialogImportVisible" v-if="dialogImportVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon name="download" class="vsd-mr-5 m-color"/>导入黑名单 </div>
            <el-form :model="importForm" label-position="left" label-width="80px" ref="importForm">
                <el-form-item label="用户类别" prop="userType" :rules="{ required: true, message: '请选择用户类别', trigger: 'change' }">
                    <el-radio-group v-model="importForm.userType">
                        <el-radio :label="0">系统黑名单</el-radio>
                        <el-radio :label="1">端口黑名单</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="端口" prop="srcAddr" v-if="importForm.userType === 1" :rules="{ required: true, message: '请输入端口', trigger: 'blur' }">
...
export default {
	...
	methods:{
        ...
        dnTemplate(){
            window.location.href=process.env.VUE_APP_APR+'/downloadTemplate?templateType=4';
        },
        importHandle(){
            this.fileName = ''
            this.importForm.userType = ''
        ...
        submitImportDialog(){
            ...
            this.$refs.importForm.validate((valid) => {
                if (valid) {
                    this.dialogFormData.append('userType', this.importForm.userType);
                    this.dialogFormData.append('srcAddr', this.importForm.srcAddr);
                    this.$api.importBlackUser(this.dialogFormData).then( res => {
        ...
        exoprtList(){
            ...
            window.location.href=process.env.VUE_APP_APR + 'exportBlackUser';
        },
        addRow(row){
        ...
        submitEditDialog(){
            this.$refs.dialogEditForm.validate((valid) => {
                if (valid) {
                    this.$api.addBlackUser(this.dialogEditForm).then( res => {
        ...
        delRow(row){
      		this.$confirm('是否确认删除吗？', '提示', {
				...
				this.$api.deleteBlackUser({id:ids}).then( res => { 
		...
		queryList(){
            this.tableData = []
			this.$api.blackUserList({...this.formInline,page:this.page}).then( res => { 
