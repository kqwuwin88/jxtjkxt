    ...
    <div class="init-content-lay" flex="dir:top">
        <div class="toolbar-lay" flex-box="0">
            <el-form inline :model="formInline">
                <el-form-item label="">
                    <el-input placeholder="分隔符" v-model.trim="formInline.keyWord" />
                </el-form-item>
                <el-form-item label="">
        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            <div class="table-wrap" flex-box="1">
                <el-table :data="tableData" class="table-border" ref="multipleTable" height="100%">
                    <el-table-column prop="keyWord" label="源字符" width=""></el-table-column>
        ...
        <el-dialog :visible.sync="dialogEditVisible" v-if="dialogEditVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon :name="this.dialogEditForm.id ? 'edit' : 'plus'" class="vsd-mr-5 m-color"/>{{this.dialogEditForm.id ? '修改分隔符' : '添加分隔符'}} </div>
            <el-form :model="dialogEditForm" label-width="70px" label-position="left" ref="dialogEditForm">
                <el-form-item label="分隔符" prop="keyWord" :rules="{ required: true, message: '请输入分隔符', trigger: 'blur' }">
                    <el-input v-model.trim="dialogEditForm.keyWord" placeholder="请输入分隔符" show-word-limit></el-input>
                </el-form-item>
        ...
        <el-dialog :visible.sync="dialogImportVisible" v-if="dialogImportVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon name="download" class="vsd-mr-5 m-color"/>导入分隔符</div>
...
export default {
	...
	methods:{
        ...
        dnTemplate(){
            window.location.href=process.env.VUE_APP_APR+'/downloadTemplate?templateType=7';
        },
        importHandle(){
            this.fileName = ''
        ...
        submitImportDialog(){
            ...
            this.$refs.importForm.validate((valid) => {
                if (valid) {
                    this.$api.importSplitword(this.dialogFormData).then( res => {
        ...
        exoprtList(){
            ...
            window.location.href=process.env.VUE_APP_APR + 'exportSplitword';
        },
        addRow(row){
        ...
        submitEditDialog(){
            this.$refs.dialogEditForm.validate((valid) => {
                if (valid) {
                    this.$api.addSplitword(this.dialogEditForm).then( res => {
        ...
        delRow(row){
      		this.$confirm('是否确认删除吗？', '提示', {
				...
				this.$api.deleteSplitword({id:row.id}).then( res => { 
		...
		queryList(){
            this.tableData = []
			this.$api.splitList({...this.formInline,page:this.page}).then( res => { 
