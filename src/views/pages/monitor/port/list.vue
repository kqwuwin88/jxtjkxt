    ...
    <div class="init-content-lay" flex="dir:top">
        <div class="toolbar-lay" flex-box="0">
            <el-form inline :model="formInline">
                ...
                <el-form-item label="">
                    <el-select v-model="formInline.portType" placeholder="端口类别">
                        <el-option label="端口类别" value="-1"></el-option>
                        <el-option label="省内端口" value="0"></el-option>
                        <el-option label="外省端口" value="1"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="">
                    <el-select v-model="formInline.filterType" placeholder="发送权限">
                        <el-option label="发送权限" value="-1"></el-option>
                        <el-option label="黑名单" value="1"></el-option>
                        <el-option label="白名单" value="2"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="">
                    <el-select v-model="formInline.filterBlackUser" placeholder="过滤黑名单用户">
                        <el-option label="过滤黑名单用户" value="-1"></el-option>
                        <el-option label="需要" value="0"></el-option>
                        <el-option label="不需要" value="1"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="">
                    <el-select v-model="formInline.filterKeyword" placeholder="过滤关键字">
                        <el-option label="过滤关键字" value="-1"></el-option>
                        <el-option label="需要" value="0"></el-option>
                        <el-option label="不需要" value="1"></el-option>
                    </el-select>
                </el-form-item>
        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            <div class="table-wrap" flex-box="1">
                <el-table :data="tableData" class="table-border" ref="multipleTable" height="100%">
                    <el-table-column prop="srcAddr" label="端口" width="120px" fixed="left"></el-table-column>
                    <el-table-column prop="corpName" label="客户名称" min-width="180px" fixed="left"></el-table-column>
                    <el-table-column prop="portType" label="端口类型" width="100px" fixed="left">
                        <template slot-scope="scope">
                            {{scope.row.portType | filterPortType}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="filterType" label="发送权限" width="100px" fixed="left">
                        <template slot-scope="scope">
        ...
        <el-dialog :visible.sync="dialogEditVisible" v-if="dialogEditVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon :name="this.dialogEditForm.id ? 'edit' : 'plus'" class="vsd-mr-5 m-color"/>{{this.dialogEditForm.id ? '修改端口' : '添加端口'}} </div>
            <el-form :model="dialogEditForm" label-width="120px" label-position="left" ref="dialogEditForm">
                <el-form-item label="端口" prop="srcAddr" :rules="{ required: true, message: '请输入端口', trigger: 'blur' }">
                    <el-input v-model.trim="dialogEditForm.srcAddr" placeholder="请输入端口" oninput="value=value.replace(/[^\d]/g,'')" show-word-limit></el-input>
                </el-form-item>
        ...
        <el-dialog :visible.sync="dialogImportVisible" v-if="dialogImportVisible" append-to-body width="500px" class="custom-dialog" :close-on-click-modal="false">
            <div slot="title"> <vsd-icon name="download" class="vsd-mr-5 m-color"/>导入端口 </div>
...
export default {
	...
	methods:{
        dnTemplate(){
            window.location.href=process.env.VUE_APP_APR+'/downloadTemplate?templateType=1';
        },
        importHandle(){
            this.fileName = ''
        ...
        submitImportDialog(){
            ...
            this.$api.importPortInfo(this.dialogFormData).then( res => {
        ...
        exoprtList(){
            ...
            window.location.href=process.env.VUE_APP_APR + 'exportPortInfo' + str;
        },
        addRow(row){
            if(row){
                this.$api.getPort({id:row.id}).then( res => { 
        ...
        submitEditDialog(){
            this.$refs.dialogEditForm.validate((valid) => {
                if (valid) {
                    const url = this.dialogEditForm.id ? 'modifyPort' : 'addPort'
        ...
        delRow(row){
      		this.$confirm('是否确认删除吗？', '提示', {
				...
				this.$api.deletePort({id:row.id}).then( res => { 
		...
		queryList(){
            this.tableData = []
			this.$api.portList({...this.formInline,page:this.page}).then( res => { 
