        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            <div class="table-wrap" flex-box="1">
                <el-table :data="tableData" v-if="refresh" class="table-border" ref="multipleTable" height="100%" :summary-method="getSummaries" show-summary>
                    <el-table-column prop="portal" label="端口" width=""></el-table-column>
                    ...
                    <el-table-column prop="provinceName" label="归属省份" width=""></el-table-column>
                    <el-table-column prop="agentName" label="所属代理商" min-width="200"></el-table-column>
                </el-table>
            </div>
...
export default {
	...
	methods:{
        ...
        exoprtList(){
            const str = `?downType=1`
            window.location.href=process.env.VUE_APP_APR + '/exportStatistPortOrProvince' + str;
        },
        searchList(){
            this.page = 1
            this.queryList()
		...
		queryList(){
            this.tableData = []
            this.refresh = false
			this.$api.smsStatistPort({...this.formInline,page:this.page}).then( res => { 
