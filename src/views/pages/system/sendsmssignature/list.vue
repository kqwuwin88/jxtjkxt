    ...
    <div class="init-content-lay" flex="dir:top">
        <div class="toolbar-lay" flex-box="0">
            <el-form inline :model="formInline" label-width="100px">
                <el-form-item label="">
                    <el-input placeholder="输入签名" v-model.trim="formInline.controlKey" />
                </el-form-item>
                <el-form-item label="">
                    <el-date-picker
                        title="日期"
        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            <div class="table-wrap" flex-box="1">
                <el-table :data="tableData" v-if="refresh" class="table-border" ref="multipleTable" height="100%" :summary-method="getSummaries" show-summary>
                    <el-table-column prop="controlKey" label="签名" width="" v-if="showDate === 0"></el-table-column>
...
export default {
	...
	methods:{
        ...
        exoprtList(){
            window.location.href=process.env.VUE_APP_APR + '/exportStatistBz?statistType=0';
        },
        searchList(){
            this.page = 1
            this.queryList()
		...
		queryList(){
            this.tableData = []
            this.refresh = false
			this.$api.statistBzOfEnt({...this.formInline,page:this.page,statistType:0}).then( res => { 
